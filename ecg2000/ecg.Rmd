...
```{r}
dataset_train <- read_table("ecg2000/data/ECG200_TRAIN.txt", col_names=FALSE)
dataset_test <- read_table("ecg2000/data/ECG200_TEST.txt", col_names=FALSE)

dataset <- rbind(dataset_train, dataset_test)

colnames(dataset) <- c("category", 1:96)
...
ggsave("ecg2000/results/ecg200.pdf", width = 10, height = 6)
```
...
generate_outliers_dataset <- function(data, outliers, method) {
    data <- data %>% rownames_to_column() %>%
        mutate(alpha = if_else(rowname %in% outliers, 1, 0.01)) %>%
        mutate(category = if_else(rowname %in% outliers, "outlier", "no outlier")) %>%
...
```{r}
ai = LstepArchetypesMod(data, k=1:8)
screeplot(ai)
```
...
```{r}
aaknn <- AAKNN(data, e = 3, narch = 8, vi = 5, vf = 50, ai=ai)
...
```{r}
trimknn <- TRIMKMEANS(data, k=2, out_rate=0.02)
...
# if alpha != 1, set to 0.01
d$alpha[d$alpha != 1] <- 0.01
```
...
ggsave("ecg2000/results/outliers.pdf", width = 14, height = 8)
```
