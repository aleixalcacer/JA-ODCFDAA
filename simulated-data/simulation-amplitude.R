...
for (cluster in clusters) {
    for (rep in 1:nrep) {
        ...
        # 
        # out_partial <- AAKNN(data, vi = 5, vf = 15, narch = 5)
        # res_partial = c(length(setdiff(out, out_partial)), length(setdiff(out_partial, out)))
    ...
    pr <-
        ...
        transform(out_rate=method %in% c("TRIM", "POND", "HDR", "LRT", "FADALARA", "TRIMKMEANS"))

    write_csv(pr, paste("simulated-data/results/csv/amplitude-", cluster, "-cluster-v2.csv", sep = ""))
}